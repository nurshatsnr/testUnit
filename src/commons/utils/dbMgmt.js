...
export const addRecord = async (recordObj, requiredFileds, tableName) => {
    ...
    try {
        ...
        log.error(`addNewReceiverInfo|error -> ${JSON.stringify(error)}`);
...
export async function updateRecord(recordObj, requiredFileds, pkFieldNm, skFieldNm, tableName, keyObj) {
    let response= {};
    try {
        log.debug("....requiredFileds.....");
        log.debug(JSON.stringify(requiredFileds));

        log.debug("....recordObj.....");
        log.debug(JSON.stringify(recordObj));
        const validationResponse = await validateRequestFields(
            recordObj,
            requiredFileds
        );
